...
if ip link show dev "$INTERFACE" >/dev/null 2>&1; then
    if managed_pid && "$BASE/bin/wg" show "$INTERFACE" >/dev/null 2>&1; then ensure_firewall || { write_status error firewall_failed; exit 1; }; "$BASE/dashboard-start.sh" >/dev/null 2>&1 || true; write_status running already_started; "$BASE/post-upgrade-health.sh" apply >/dev/null 2>&1 || true; exit 0; fi
...
"$BASE/post-upgrade-health.sh" apply >/dev/null 2>&1 || true
exit 0
